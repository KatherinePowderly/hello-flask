...
(template_dir), autoescape=True) #cgi on jinja

app = Flask(__name__)
app.config['DEBUG'] = True

@app.route("/") ##hello_form template
def index():
...
def validate_time():
    ...
    if not minutes_error and not hours_error:
        time = str(hours) + ':' + str(minutes) ##convert back to str
        return redirect('/valid-time?time={0}'.format(time)) ##redirect
    else:
